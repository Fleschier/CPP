...
void Merge_Sort(int n, int lst[], int &compare, int &move) {
	compare = 0; move = 0;
	int *Tmp = new int[n];			//new是用方括号....圆括号会引发异常
	if (Tmp != NULL) {
...
//快速排序
//经过调试最终发现出现的问题
//这里快速排序的异常：stack overflow：栈溢出
//简单来说就是分给程序的递归栈满了导致递归失败所以直接退出了
//这里我用我自己写的quick sort代替了原本的quick sort，可以看出比较的次数非常庞大，与插入排序相当，可能是选取的分割点不好
//我写的quicksort在4000左右的时候就会出现栈溢出，而原本的连长度为100的数组都会出现栈溢出的异常，也不知道为什么
...
int partition_and_swap(int Array[], int head, int rear, int &compare, int &move) {
	int division = Array[rear];					//以末尾的元素作为分割元素
	int i = head - 1;
...
int main() {
	...
	int test_num = 0;  //测试次数		//赋值为1也会被覆盖掉，没有作用
	cout << "请输入测试次数:";
	cin >> test_num;

	while (test_num) {
		int n_num = 0;			//??首先赋值为20完全没有意义，会被下面的cin覆盖掉。
		cout << "请输入文件要存储的整数的个数：" << endl;
		cin >> n_num;
		int *lst = new int[n_num + 1];			//new方法创建数组用方括号不是圆括号。。。。。。
		Creat_File(sorted_num, reversed_num, random_num, n_num);
		Read_File(reversed_num, lst);
		test(n_num, lst);	
		test_num--;
		delete[] lst;			//// 释放整个数组，new如果带[] 则delete也需要带[]
	}
	/*int m = 0, n = 0;
